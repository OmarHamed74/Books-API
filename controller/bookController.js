...
exports.getBookList = async (req, res) => {
  try {
    ...
    return res.status(500).send({ error: "Falid to get book" });
  }
};
...
exports.getBookDetails = async (req, res) => {
  try {
    ...
    console.log("Error : " + err);
    return res.status(500).send({ error: "Falid to get book details" });
  }
};
...
exports.updateBook = async (req, res) => {
  try {
    ...
    if (!bookId || !title || !author || !publisher || !storeCode) {
      return res.status(500).send({
        error:
          "bookId, title , author , publisher , storeCode are required , can not empty",
      });
    }
    ...
    return res.status(201).send(`Successfully update book : ${title}`);
  } catch (err) {
    console.log("Error : " + err);
